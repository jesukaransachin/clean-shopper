...
4. **Run the schema**: open your Supabase project's SQL editor and run, in order:
   ...
   - `supabase/schema-saved-products.sql` (save-to-list persistence: `saved_products`) — run this *in addition to* the two above, not instead of them.
5. **Seed the catalog**:
   ```bash
   npm run seed
   ```
   Inserts ~27 products across Personal Care, Home Cleaning, and Baby Care into your Supabase project. (Cart and saved products both start empty per-browser — there's nothing to seed there.)
6. **Run the app**:
   ```bash
   npm run dev
   ```
...
7 tables are executed: `brands`, `products`, `certifications`, `product_certifications` (`supabase/schema.sql`), `shoppers` and `cart_items` (`supabase/schema-cart.sql`), and `saved_products` (`supabase/schema-saved-products.sql`). The rest of `docs/database-schema.sql` (`ingredients`, `shopper_avoided_ingredients`, `shopper_trusted_brands`, `shopper_preferred_certifications`, `research_*`, `comparisons`) is still just a proposal — nothing in the frontend has wired UI for those yet (no preferences UI, no ingredient-avoid matching). See `docs/database-schema.md` for the full rationale and the two documented deviations in the original 4-table cut (a `reason` column added directly to `products`, and alert-style badges stored as static certification rows rather than computed at query time) — `shoppers`/`cart_items`/`saved_products` have no such deviations, executed exactly as originally documented.

## Anonymous identity (cart + saved products persistence)

No user accounts/auth in V1 (see `docs/database-schema.md` §1). The cart and saved products are both scoped to an anonymous, per-browser identity instead:

- `src/lib/session.js` generates a random UUID on first visit and stores it in `localStorage` (`cleanShopperSessionToken`).
- Every cart and saved-products request sends it as the `X-Session-Token` header.
- `server/middleware/session.js` resolves that token to a `shoppers` row, creating one on first use.

This means the cart and saved products persist across reloads on the same browser, but not across devices/browsers, and clearing site data resets both. If real accounts get added later, `shoppers` is exactly where an `email`/`auth_provider_id` column would go — nothing else in the schema needs to change.
...
**Saved products** (all require an `X-Session-Token` header — 400 if missing)
- `GET /api/saved-products` — this shopper's saved products, shaped like the products endpoint above.
- `POST /api/saved-products` — body `{ productId }`. Upserts, so re-saving an already-saved product is a no-op rather than an error.
- `DELETE /api/saved-products/:productId` — removes the saved product.

## Known simplification: optimistic updates, no rollback

`src/lib/CartContext.jsx` and `src/lib/SavedProductsContext.jsx` both update local state immediately on every action (add/increment/decrement/remove, save/unsave) and fire the corresponding API call in the background. If that call fails, the error is logged to the console — the UI does not roll back or show an error toast. This keeps the interaction feeling instant, but means a failed persist is silently inconsistent between what's on screen and what's in the database until the next full reload (which refetches from the server and would correct it). Fine for a course demo; would need real reconciliation (optimistic-update rollback, retry, or a "sync failed" indicator) for production use.
